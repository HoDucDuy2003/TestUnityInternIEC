...
public class Board
{
    ...
    private int extraRowSize = 5;

    private Cell[,] m_cells;
    private Cell[] m_extraRowCells;
    ...
    public Cell[] ExtraRowCells => m_extraRowCells;
    ...
    public Board(Transform transform, GameSettings gameSettings)
    {
        ...
        m_extraRowCells = new Cell[extraRowSize];
    ...
    private void CreateBoard()
    {
        ...
        // Add extra row
        int extraRowY = -1;
        for (int x = 0; x < extraRowSize; x++)
        {
            GameObject go = GameObject.Instantiate(prefabBG);
            go.transform.position = origin + new Vector3(x - (extraRowSize - boardSizeX) * 0.4f, extraRowY, 0f);
            go.transform.SetParent(m_root);

            Cell cell = go.GetComponent<Cell>();
            cell.Setup(x, extraRowY);
            m_extraRowCells[x] = cell;
        }
    ...
    internal void Fill()
    {
        ...
        int extraTypeIndex = UnityEngine.Random.Range(0, 7);
        NormalItem.eNormalType extraType = types[extraTypeIndex];
        typeCount[extraType] = 6;
        ...
        foreach (var type in types)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                item.Setup(m_root.transform, type);
                m_cells[pos.x, pos.y].Assign(item);
                if (item.View != null)
                {
                    item.SetViewPosition(m_cells[pos.x, pos.y].transform.position);
        ...
        Debug.Log($"Total items filled: {totalCells}");*/
    }
    ...
    public bool HasItemsOnMainBoard()
    {
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                Cell cell = m_cells[x,y]; // Lấy ô từ m_cells trong Board
                if (!cell.IsEmpty) // Nếu ô còn item
                {
                    return true; // Còn item trên bảng chính
                }
            }
        }
        return false; // Không còn item trên bảng chính
    }
}
